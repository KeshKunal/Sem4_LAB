...
Goal:
- Simulate indexed file allocation.

Logic:
1. Allocate an index block for each file.
2. Store the list of data blocks in the index.
3. Display file table with index and blocks.

Key Variables:
- used[] -> block usage bitmap
- files[] -> file table (index block + data blocks)
...
/*
Program Name: Indexed File Allocation
Aim: Write a C program to simulate Indexed file allocation technique.
Algorithm:
1. Read file name, index block, and data blocks.
2. Validate blocks and allocate.
3. Display file table.
Compilation: gcc 11a_indexed_file_allocation.c -o indexed
Execution: ./indexed
*/
